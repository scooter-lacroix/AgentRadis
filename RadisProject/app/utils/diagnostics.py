...
class DiagnosticInfo:
    ...
    def get_diagnostic_report(self, include_severity: Optional[List[SeverityLevel]] = None) -> Dict[str, Any]:
        """
        Generate a comprehensive diagnostic report.
        
        Args:
            include_severity: Optional list of severity levels to include (all if None)
            
        Returns:
            A dictionary containing categorized diagnostic information
        """
        filtered_errors = self.errors
        if include_severity:
            severity_strings = [str(s) for s in include_severity]
            filtered_errors = [e for e in self.errors if e.get("severity") in severity_strings]
        ...
        return {
            ...
            "last_tool_execution": self.last_tool_execution,
            "runtime_states": self.runtime_states,
            "runtime_state_count": len(self.runtime_states),
        }
