...
class StrReplaceEditor(BaseTool):
    ...
    def description(self) -> str:
        return """Replace text in files using string or regex patterns.
This tool allows for simple text replacement or more complex regex-based substitutions."""
    ...
    async def run(self, **kwargs) -> Dict[str, Any]:
        ...
        if replace is None:
            return {"status": "error", "error": "No replacement text provided"}
            ...
            if use_regex:
                ...
                if not case_sensitive:
                    ...
                    if occurrence > 0:
                        ...
                        pattern = re.escape(search)
                        content = re.sub(
                            pattern, repl_func, content, flags=re.IGNORECASE
                        )
                else:
                    if occurrence > 0:
                        # Replace specific occurrence
